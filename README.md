...
- **名称缓存（提速）**：`address→用户名`、`conditionId→市场名/网址` 会做**本地 TTL 缓存**（localStorage，用户名 7 天 / 市场名 30 天），跨刷新复用；只缓存「有结果」的正向数据，绝不把「查不到」钉死。命中市场名缓存可**跳过整份市场索引构建**（省去数千条 GraphQL 拉取）。清缓存：邀请关系卡片的「清空」按钮会一并清掉名称缓存。
- **地址备注 / 标签**：表格与邀请结果里每个地址旁有 🏷，点击可给该地址加本地备注（如「大户 A」「高频对手方」）。备注参与表格搜索、并随 CSV 导出（「备注」列）。纯前端本地存储，不上传。
